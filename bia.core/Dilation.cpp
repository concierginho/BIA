...
void BIA::Dilation::PerformOperation(Bitmap* bitmap, nlohmann::json& json)
{
   ...
   for (int i = left_top_x; i < right_bot_x - 1; i++)
   {
      for (int j = left_top_y; j < right_bot_y - 1; j++)
      {
         ...
         bool isAnyBlack = false;
         for (auto& neighbour : neighbours)
         {
            if (buffer[neighbour] == black)
            {
               isAnyBlack = true;
               break;
            }
...
std::vector<int> BIA::Dilation::GetNeighbours(int x, int y, int img_width, int img_length, Bitmap* bitmap)
{
